...
void DW_arm926_no_icm_example::InitInstances() {
    ...
    arm926 = new DW_arm926ejs("arm926", 2, true, 1, true, BigEndian, ProgName, Debugger);
    ...
    int tmp_intr_gen_SlaveID = 3;
    ccss_set_parameter("intr_gen.SlaveID", tmp_intr_gen_SlaveID);
    int tmp_intr_gen_ReadWaitStates = 0;
    ccss_set_parameter("intr_gen.ReadWaitStates", tmp_intr_gen_ReadWaitStates);
    int tmp_intr_gen_WriteWaitStates = 0;
    ccss_set_parameter("intr_gen.WriteWaitStates", tmp_intr_gen_WriteWaitStates);
    bool tmp_intr_gen_ReadOnly = false;
    ccss_set_parameter("intr_gen.ReadOnly", tmp_intr_gen_ReadOnly);
    bool tmp_intr_gen_DisableMonitor = false;
    ccss_set_parameter("intr_gen.DisableMonitor", tmp_intr_gen_DisableMonitor);
    bool tmp_intr_gen_DoTrace = DoTrace;
    ccss_set_parameter("intr_gen.DoTrace", tmp_intr_gen_DoTrace);
    intr_gen = new DW_IntrGen<ahb_slave_if >("intr_gen", IntrGenStartAddr, IntrGenEndAddr, "", BigEndian);
    ...
    ahb_bus->slave_port(*intr_gen);
    arbiter->ahb_wt_mask(C189);
    ResetGen->Out(High);
    ...
    intr_gen->INITRAM(INITRAM);
    arbiter->pause(Low1);
    FalseGen->Out(Low1);
    intr_gen->VINITHI(VINITHI);
    ...
    arm926->nIRQ(nIRQ);
    intr_gen->nIRQ(nIRQ);
    intr_gen->nRESET(nRESET);
    arm926->nRESET(nRESET);
}

void DW_arm926_no_icm_example::DeleteInstances() {
    ...
    if (intr_gen) {delete intr_gen; intr_gen = 0;}
